...
function snake() {
	...
	$("div#tabs ul#listOfTabs").append("<li><a href='#tab" + num_tabs + "'>Snake</a></li>");
	$("div#tabs").append("<div id='tab" + num_tabs + "'><canvas id=\"canvas\" width=\"450\" height=\"450\"></canvas></div>");
	...
	var snake = {
		...
		deplacer: function(d) {
			...
			if (check_collision(temp.x, temp.y, this.snake_array)) {
				game.restart();
			} else if (check_collision(temp.x, temp.y, this.snake_array) || temp.x == -1 || temp.x == canvas.width / sizeOfRectangle || temp.y == -1 || temp.y == canvas.height / sizeOfRectangle) {
				game.restart();
			} else {
